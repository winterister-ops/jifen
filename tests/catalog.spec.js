...
test.describe('任务与奖励管理', () => {
  ...
  test('首页按分值从低到高排列', async ({ page }) => {
    const pts = await page.locator('.earn-item .catalog-pts').evaluateAll(els =>
      els.map(el => parseInt((el.textContent || '').replace('+', ''), 10))
    );
  ...
  test('自定义目录会写入 localStorage', async ({ page }) => {
    ...
    const stored = await page.evaluate(({ taskName, rewardName }) => {
      ...
      return {
        hasTask: !!task && !task.preset && task.pts === 4 && task.enabled && !task.group,
