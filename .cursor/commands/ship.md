...
2. 如改了 `.js` / `.html` / `.css`，先跑测试（Playwright 自带 Chromium）。沙箱每次运行的浏览器缓存是隔离的，所以要在同一条命令里先装再跑：`npx playwright install chromium >/dev/null 2>&1 && npm test`。仅改 README / 许可 / 注释可跳过并说明。
