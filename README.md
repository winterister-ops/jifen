...
- **邮箱登录**：打开网页需先登录家长账号；新账号在 Firebase 控制台创建，不支持前端自助注册
- **忘记密码**：输入邮箱获取验证码，在本页填写验证码并设置新密码；点击邮件中的重置链接可自动跳回并填入验证码

## 账号与忘记密码

- 新账号在 Firebase Console → **Authentication → Users → Add user** 创建
- 忘记密码使用 Firebase 邮件验证码（`oobCode`），流程为：发送验证码 → 填写验证码 → 设置新密码
- 若本地测试收不到邮件，请检查：
  1. 邮箱是否已在 Firebase 中注册
  2. 垃圾箱 / 促销邮件文件夹
  3. Firebase Console → **Authentication → Settings → Authorized domains** 是否包含 `localhost`
  4. Firebase Console → **Authentication → Templates → Password reset** 中，将「自定义操作 URL」设为 `http://localhost:8080/`（本地）或你的线上地址，这样点击邮件链接会跳回应用并自动填入验证码
